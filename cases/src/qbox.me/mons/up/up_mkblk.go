...
type UpMkBlk struct {
	...
	Ip        string `json:"ip"`
	IpInclude string `json:"ip_include"`
...
func (self *UpMkBlk) Init(conf []byte) error {
    ...
    conf, err = ioutil.ReadFile(self.IpInclude)
	if err != nil {
		return err
	}
...
func (self *UpMkBlk) doTestMkBlk(host, ip string) (msg string, err error) {
	...
	msg = self.GenLog(host+"_resumableput", begin, end, duration)
	if err != nil || code != 200 {
		return
	}
	return
...
func (self *UpMkBlk) Mon() (msg string, err error) {
    ...
    for name, ip := range self.Ips {
        fmt.Println(name, " : ", ip)
        msg1, err := self.doTestMkBlk(name, self.Ip)
        if err == nil {
