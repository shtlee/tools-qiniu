...
func NewService(c *Config, t http.RoundTripper, taskQsize, threadSize int) (s Service, err error) {
    ...
    s = Service{tasks, c, httputil.Client{c, client}}
    return 
}
...
func (r Service) resumableBlockput(
    ...
    offbase := int64(blockIdx) << r.Config.BlockBits
    h := crc32.NewIEEE()
...
func (r Service)BlockCount(fsize int64) int {
    blockMask := int64((1 << r.Config.BlockBits) - 1)
    return int((fsize + blockMask) >> r.Config.BlockBits)
}
...
func (r Service) Put(
    ...
    wg.Add(blockCnt)
    last := blockCnt - 1
    blockSize := 1 << r.Config.BlockBits
    var failed bool
        ...
        if checksums[i] == "" {
            ...
            task := func() {
                ...
                ret, code, err2 := r.resumableBlockput(
                    f, blockIdx, blockSize1, r.Config.PutChunkSize, PUT_RETRY_TIMES, &progs[blockIdx], chunkNotify)
                if err2 != nil {
...
func (r Service) Mkfile(
    ...
    code, err = r.Conn.CallWithBy(
        "up", ret, r.Config.HostIp["up_ip"]+cmd+httputil.EncodeURI(entry)+"/fsize/"+strconv.FormatInt(fsize, 10)+params,
